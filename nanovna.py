...
class NanoVNAV2(NanoVNA):
    ...
    def set_frequencies_by_list(self, lst=[1e6]):
        self.points = len(lst)
        self.sweepData = [[0.,0.]] * self.points
        self._frequencies = np.array(lst)
    ...
    def _scan(self):
        ...
        if nBytes != len(arr):
            print("expected %d bytes, got %d" % (nBytes, len(arr)))
    ...
    if plot or opt.save:
        ...
        if opt.scan or opt.points > 101:
            ...
            if opt.start or opt.stop:
                nv.set_sweep(opt.start, opt.stop, opt.points)
